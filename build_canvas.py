# -*- coding: utf-8 -*-
"""
Builds raw/decision-canvas.html: the Centene "Enterprise Application Value
Inversion" interactive decision canvas.

Format: follows the same click-driven tab/panel architecture as
collab-hubs/kaiser-risant-value-inversion/raw/decision-canvas.html (Problem /
Status Quo / The Big Question / Opportunity / New Approach / Plan), reskinned
with Centene's brand colors (pulled live from centene.com 9/8/2026) and
populated with Centene's own TAM figures (Estimated Software Spend export,
9/8/2026), current tech-stack vendor data (Solution Infrastructure Tech Stack
Overview export, TPID CENTENE CORPORATION - 2335114), and Jake-supplied
per-user license rates.
...
CENTENE_COLOR = f"data:image/jpeg;base64,{LOGOS['centene_color']}"
CENTENE_WHITE = f"data:image/png;base64,{LOGOS['centene_white']}"
...
# ============================================================ MODEL =======
# Centene "Estimated Software Spend" TAM (image supplied by Jake, 9/8/2026).
TAM_CATEGORY_TOTALS = {
    "App Development": 106.759017,
    "CRM": 336.429418,
    "ERP": 410.529762,
}
TAM_TOTAL = sum(TAM_CATEGORY_TOTALS.values())  # ~$853.7M

# Illustrative vendor-name examples per category, curated from Centene's own
# Solution Infrastructure Tech Stack Overview export (TPID CENTENE
# CORPORATION - 2335114, HQ scope) down to legacy business applications that
# are plausible Power Apps / Dynamics 365 replacement candidates. AI tools,
# dev/infra tools, security tools, and general productivity/collaboration
# software are excluded as out of scope for this narrative.
CATEGORY_VENDOR_EXAMPLES = {
    "App Development": ["UiPath", "Automation Anywhere", "Blue Prism", "ServiceNow", "Pega"],
    "CRM": ["Salesforce", "Pega", "Genesys", "Avaya", "Marketo", "HubSpot"],
    "ERP": ["SAP", "Oracle", "JD Edwards", "Infor", "Workday", "Kronos"],
}

CENTENE_EMPLOYEES = 61_100  # FY2025 10-K, cross-checked 9/8/2026
POWERAPPS_RATE = 2.99       # Jake-supplied, per user/month
D365_RATE = 28.97           # Jake-supplied, per user/month
...
FLOOR_M, CEILING_M = floor_ceiling_m(CENTENE_EMPLOYEES)
...
# Centene model instead of a static per-account screenshot, since these
# numbers must vary per customer). Total cost of ownership = the remaining,
# un-migrated portion of the TAM footprint each year as a phased 10%/year
# migration progresses (narrow-licensed / floor scenario). Value realization
# is illustrative and unitless, shaped only to cross the TCO curve visually.
_TCO_YEARS = list(range(0, 11))
TCO_BY_YEAR = [TAM_TOTAL - FLOOR_SAVINGS_POTENTIAL * n / 10 for n in _TCO_YEARS]
VALUE_BY_YEAR = [900 * (n / 10) ** 1.6 for n in _TCO_YEARS]
...
:root {
  ...
  --cp-text-soft: #6b7c8f;
  --cp-accent: #00598C;
  --cp-accent-hover: #00436A;
  --cp-accent-soft: rgba(0, 89, 140, 0.10);
  --cp-accent-fg: #ffffff;
  --cp-success: #2C8641;
  --cp-danger: #A14545;
  --cp-warning: #f4a100;
  ...
  --cp-highlight: rgba(0, 185, 242, 0.16);
  --cp-purple: #262768;
  --cp-green: #2C8641;
  --cp-teal: #008575;
  --cp-gold: #CB187D;
  --cp-navy-fixed: #14152E;
  --cp-navy-fixed-2: #1b1c47;
}
html[data-theme="dark"] {
  color-scheme: dark;
  --cp-bg: #14152E;
  --cp-bg-elevated: #0e0f24;
  --cp-surface: #1b1c47;
  --cp-surface-soft: #262768;
  --cp-border: #33346e;
  --cp-border-strong: #4d4f8f;
  --cp-text: #e8eef5;
  --cp-text-muted: #b9c0e0;
  --cp-text-soft: #c9d1ec;
  --cp-accent: #00B9F2;
  --cp-accent-hover: #33c7f5;
  --cp-accent-soft: rgba(0, 185, 242, 0.16);
  --cp-accent-fg: #0B1730;
  --cp-success: #5fd88a;
  --cp-danger: #ef8b8b;
  --cp-warning: #ffc44d;
  ...
  --cp-overlay: rgba(20, 21, 46, 0.88);
  --cp-panel: rgba(27, 28, 71, 0.74);
  --cp-panel-strong: rgba(27, 28, 71, 0.97);
...
/* architecture stack (IQ platform, hyperscaler compare) */
...
<nav class="tabs" aria-label="Decision canvas sections">
  ...
  <button class="tab-button" data-tab="opportunity" data-tip-title="Opportunity" data-tip="Ground the argument in Centene's own numbers: the $853.7M TAM and the 10-year savings model.">
...
<section id="problem" class="panel active" aria-labelledby="problem-title">
  ...
  <div class="card">
    ...
    <div class="footer-strip">This is a per-vendor tax. Centene likely pays it more than once.</div>
  </div>
</section>
"""
...
<section id="status-quo" class="panel" aria-labelledby="status-quo-title">
  ...
  <div class="card">
    ...
    <div class="footer-strip">None of this requires ripping anything out. AI can extend what you already run today (Salesforce, Workday, Pega, and more) on one shared foundation, with no separate layer to license or maintain.</div>
  </div>

  <div class="card">
    <div class="card-header"><h3>Fragmented context is why most enterprise AI never pays off</h3><p>The data proves it. Every handoff in the member journey feels it.</p></div>
    <div class="stat-grid">
      ...
      <div class="stat-card"><div class="stat-num">$20B</div><p>in annual U.S. healthcare savings still on the table from replacing manual, disconnected administrative transactions between payers and providers (CAQH Index, 2025)</p></div>
      <div class="stat-card" style="border-top-color:var(--cp-teal)"><div class="stat-num" style="color:var(--cp-teal)">13 hrs/wk</div><p>average clinician and staff time lost industry-wide to prior authorization and manual documentation (AMA, 2024), friction Centene's own utilization management workflow can help remove</p></div>
    </div>
    <div class="handoff-row">
      <div class="handoff-card"><b>Enrollment</b>Member enrolls; contact center can't see their eligibility history across systems <span class="x">&#10005;</span></div>
      <div class="handoff-card"><b>Prior Authorization</b>UM staff re-enter clinical data that claims and care-management systems don't share <span class="x">&#10005;</span></div>
      <div class="handoff-card"><b>Claims</b>Claims team manually reconciles data across systems that don't talk to each other <span class="x">&#10005;</span></div>
      <div class="handoff-card"><b>Care Management</b>Case managers coordinate outreach without visibility into recent claims or authorization history <span class="x">&#10005;</span></div>
    </div>
    <div class="footer-strip">The problem was never the model. It's what the model can't see, and it costs trust at the exact moments Centene's mission depends on it most.</div>
    <p class="source-note">Industry-wide research findings, not Centene-specific data. Member-journey pattern is illustrative, based on common cross-system handoff points in large managed care organizations.</p>
  </div>
</section>
"""
...
_TAM_PICK_COLORS = {"App Development": "#742774", "CRM": "#00598C", "ERP": "#2C8641"}
_TAM_PICK_LABELS = {
    "App Development": "LOW-CODE APP DEV &amp; AUTOMATION",
...
<section id="opportunity" class="panel" aria-labelledby="opportunity-title">
  <div class="panel-head">
    <h2 id="opportunity-title">What this is worth to Centene.</h2>
    <p>Grounded in Centene's own estimated software spend, not generic industry benchmarks.</p>
  </div>

  <div class="card">
    <div class="card-header"><h3>The AI Business Applications TAM at Centene</h3><p>~{fmt_big_m(TAM_TOTAL)} of AI-BPS-addressable third-party software spend across Centene's application portfolio. Click a category for representative vendors.</p></div>
    <div class="section-grid">
  ...
  <div class="card">
    ...
    <p class="source-note">Figures are NET savings: third-party spend avoided minus the Microsoft license cost incurred, modeled against Centene's full 61,100-employee headcount at $2.99/user/month (Power Apps) and $28.97/user/month (Dynamics 365). Illustrative, not a quote. Actual pace, mix, and licensed seat count will vary by workload.</p>
  </div>

  <div class="card">
    <div class="card-header"><h3>Apps are the real estate: lower cost to own, more capacity for mission</h3><p>What this frees up for Centene.</p></div>
    <div class="stat-grid">
      <div class="stat-card" style="border-top-color:var(--cp-green)"><div class="stat-num" style="color:var(--cp-green)">{fmt_big_m(TENYR_CEILING_CUMULATIVE)}&ndash;{fmt_big_m(TENYR_FLOOR_CUMULATIVE)}</div><p>saved over 10 years by migrating to Microsoft: capacity to reinvest in member experience, not licensing</p></div>
      ...
      <div class="stat-card" style="border-top-color:var(--cp-teal)"><div class="stat-num" style="color:var(--cp-teal)">Time back</div><p>AI agents take on high-volume administrative and member-service tasks, freeing staff to focus on members, not systems</p></div>
    </div>
    <div class="footer-strip">Every dollar not spent on licensing markups and vendor sprawl is a dollar available for member experience, affordability, and Centene's mission to transform the health of the communities it serves.</div>
  </div>
</section>
"""
...
<section id="new-approach" class="panel" aria-labelledby="new-approach-title">
  ...
  <div class="card">
    <div class="card-header"><h3>Proof it works</h3><p>This isn't theoretical. Grounded AI is already changing how managed care gets delivered; these are industry-wide patterns, not Centene-specific results yet.</p></div>
...
<section id="plan" class="panel" aria-labelledby="plan-title">
  ...
  <div class="card">
    ...
    <div class="get-there-list">
      ...
      <div class="get-there-step">
        <span class="get-there-badge" style="background:#00598C">2</span>
        <div><h5>Scrutinize with intention</h5><p>Identify where total cost of ownership can actually come down.</p></div>
      </div>
      <div class="get-there-step">
        <span class="get-there-badge" style="background:#2C8641">3</span>
  ...
  <div class="close-card">
    <div class="close-logos">
      <img src="{CENTENE_WHITE}" alt="Centene Corporation" style="height:30px" />
    </div>
    ...
    <div class="next-step-box">
      ...
      <p>A complimentary Application Portfolio TCO + AI Readiness Assessment, sized to Centene's actual application estate and data landscape.</p>
    </div>
  </div>

  <p class="source-note" style="margin-top:8px">All cost figures on this canvas (license increase %, implementation cost, maintenance cost, license savings %, TAM figures) are illustrative ranges based on Microsoft field experience and Centene-provided data as of 2026. They are not audited, vendor-published, or contractually guaranteed. The &ldquo;3&ndash;10% YoY SaaS renewal uplift&rdquo; figure and the term &ldquo;SaaSpocalypse&rdquo; reflect widely discussed industry commentary, not audited vendor data. Recommended next step: a joint TCO workshop using actual vendor invoices and contract terms to replace these illustrative figures with validated numbers.</p>
</section>
"""
...
const stackThenItems = [
  { label: "User Interfaces", weight: 34, color: "#00598C", detail: "Screens and forms were the product for two decades. That's exactly the layer AI agents route around first." },
...
const stackNowItems = [
  { label: "Data", weight: 34, color: "#14152E", textColor: "#fff", detail: "Whoever controls clean, unified data controls what every future AI agent can actually do. This is the new center of gravity." },
  { label: "Context & Knowledge", weight: 24, color: "#00598C", detail: "Policies, history, and institutional memory are what make an answer trustworthy instead of merely plausible." },
  { label: "Compute", weight: 13, color: "#2C8641", textColor: "#fff", detail: "Every AI action costs compute. Whoever owns the compute layer captures the margin, or pays someone else's markup for it." },
  { label: "AI Orchestration", weight: 11, color: "#262768", textColor: "#fff", detail: "Coordinating agents across systems, not just answering one question at a time, is where the next wave of value concentrates." },
];
...
const fiveThings = [
  { title: "How your people work", meta: "Work IQ", color: "#00598C", detail: "Roles, workflows, files, meetings, and the day-to-day rhythm of the organization. This is the context most Copilots already have some access to." },
  { title: "How you understand your members", meta: "Customer IQ", color: "#B8792A", detail: "A unified member profile with engagement signals: enrollment, benefits, and history, connected instead of scattered across a contact center, a CRM, and a marketing platform." },
  { title: "How your business runs", meta: "Fabric IQ", color: "#2C8641", detail: "Data, semantics, rules, and the live signals the business runs on: claims, eligibility, prior authorization, finance. Most AI never reaches this layer at all." },
  { title: "What your org knows", meta: "Foundry IQ", color: "#262768", detail: "Policies, documents, research, and the knowledge locked in institutional memory; usually scattered across SharePoint, wikis, and people's heads." },
...
const tamItems = [
  { title: "Low-Code App Dev & Automation", meta: "$106.8M addressable", color: "#742774",
    vendors: ["UiPath", "Automation Anywhere", "Blue Prism", "ServiceNow", "Pega"],
  ...
  { title: "Member & Engagement Platforms", meta: "$336.4M addressable", color: "#00598C",
    vendors: ["Salesforce", "Pega", "Genesys", "Avaya", "Marketo", "HubSpot"],
    detail: "Core CRM, marketing, and sales software spend: the largest single category, spanning member engagement, contact center, and marketing platforms that each carry their own rented-compute markup and isolated data layer." },
  { title: "Finance & Operations", meta: "$410.5M addressable", color: "#2C8641",
    vendors: ["SAP", "Oracle", "JD Edwards", "Infor", "Workday", "Kronos"],
    detail: "ERP, HR, and procurement systems spend: the single largest addressable category, and prime candidates for consolidation onto one shared data + AI foundation." },
];
...
const proofItems = [
  ...
  { title: "Automated claims & payment integrity", detail: "Reduces manual rework on denials and claims processing by grounding requests in data that already exists across systems." },
  { title: "AI-assisted utilization review", detail: "Reduces the manual chart review and documentation burden on utilization management staff, speeding decisions for members and providers alike." },
  { title: "Member risk stratification & care management", detail: "Case managers and AI agents share the same member risk profile in real time, closing gaps in outreach instead of after the fact." },
  { title: "Fraud, waste & abuse detection", detail: "AI surfaces anomalous billing patterns across claims in real time, protecting program integrity and taxpayer dollars." },
];
...
function buildProofGrid() {
  ...
  if (!el) return;
  const palette = ["#00598C", "#2C8641", "#262768", "#CB187D", "#1464C8", "#008575"];
...
function wireLogoTheme() {
  ...
  if (!dark) return;
  const centene = document.getElementById("logo-centene");
  if (centene) centene.src = window.__CENTENE_LOGO_WHITE__;
}
...
<title>Centene Corporation &middot; The Enterprise Application Value Inversion</title>
<style>{CSS}</style>
<script>
window.__CENTENE_LOGO_WHITE__ = "{CENTENE_WHITE}";
</script>
</head>
<body>
<div class="app">
...
<footer class="hub-footer">&copy; 2026 Microsoft &middot; Prepared for Centene Corporation &middot; Not for external redistribution</footer>
<script>
{JS}
</script>
</body>
